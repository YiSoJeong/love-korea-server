...
const getAreaList = (BASE_URL, API_KEY) => async (req, res, next) => {
  ...
  axios
    .get(
      `${BASE_URL}/areaBasedList?ServiceKey=${API_KEY}&MobileOS=AND&MobileApp=love-korea&_type=json&numOfRows=${AREALIST_NUM}&areaCode=${areacode}`,
    )
    .then((response) => {
    ...
    .catch((error) => {
      res
        .status(400)
        .json({ message: '선택한 지역의 관광지를 불러올 수 없습니다.' });
    });
};
...
const getLocationList = (BASE_URL, API_KEY) => async (req, res, next) => {
  ...
  axios
    .get(
      `${BASE_URL}/locationBasedList?ServiceKey=${API_KEY}&MobileOS=AND&MobileApp=love-korea&_type=json&numOfRows=${AREALIST_NUM}&mapX=${mapX}&mapY=${mapY}&radius=${radius}`,
    )
    .then((response) => {
      const AreaList = response.data.response.body.items.item;
      result.data = AreaList.map((x) => {
...
const getDetailInfo = (BASE_URL, API_KEY) => async (req, res, next) => {
  ...
  axios
    .get(
      `${BASE_URL}/detailCommon?ServiceKey=${API_KEY}&MobileOS=AND&MobileApp=love-korea&_type=json&numOfRows=${AREALIST_NUM}&contentId=${id}&defaultYN=Y&firstImageYN=Y&areacodeYN=Y&addrinfoYN=Y&mapinfoYN=Y&overviewYN=Y`,
    )
    .then((response) => {
    ...
    .catch((error) => {
      res
        .status(400)
        .json({ message: '해당 관광지 정보를 불러올 수 없습니다.' });
    });
};
